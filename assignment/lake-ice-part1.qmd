...
#Add this line because it would read the ice_off_doy as a character instead of numbers
...
ggplot(sunapee, aes(x = year, y = ice_off_doy)) + 
  ...
  geom_line() + 
  labs(x = "Year", 
       y = "Ice-Off Day of Year (DOY)", 
       title = "Ice-Off Day of Year at Lake Sunapee") + 
  theme_classic()
```
...
**Answer 2:**
I expect the slope to be negative because climate change will cause the lakes to lose their ice earlier in the year than normal. Due to the lake losing their ice faster than normal, the DOY value should decrease over time which would make the slope negative. 
...
```{r}
fit <- lm(data = sunapee, ice_off_doy ~ year)
...
**Answer 3:**
The slope of the model is negative which match my expectations from earlier. This matches my expectations of the warmer temperatures making the ice melt earlier in the year.

# Is our model any good?
The R squared value is very low meaning that the model is not very good and only accounts for 21% of the variation. It shows some of the variation but not much. 
...
**Answer 4:**
The R2 value is 0.2055 which is very low and explains very little variation in the data. 
...
**Answer 6:**
The histogram shows that most of the residuals are around 0 and the graph has somewhat of a bell shape meaning that it is normally distributed. There might be a slight left skew down towards -30, but for the most part it is normally distributed. 
...
**Answer 7:**
The p value is 2.5 x 10^-9 power which is much smaller than 0.05. Since the p value is smaller than 0.05, the slope is statistically relevant and shows a significant relationship between year and ice-off day of year at Lake Sunapee.
...
**Answer 8:**
Since the slope is −0.10602 days per year, it would take around 66.020 years for a week shift in ice-off timing. This is helpful due to a week shift in seasonal timing can signifigantly affect ecosystems, species life cycles, and lake productivity.
...
**Answer 9:**
I am not confident in this linear model for further analysis. The slope is significant due to the small p value, and the residuals appear roughly normal and centered around zero. However, the R squared value indicates that year explains only about 21% of the variation in ice‑off timing. While the model shows that the slope and residuals are normal, the R squared value means that the model on explains 21% of the varation which is not very significant. 
...
ggplot(sunapee, aes(x = year, y = ice_off_doy)) + 
         geom_line() + 
         geom_point() +
         geom_abline(intercept = 318.94856, 
                     slope = -0.10602, 
                     color = "blue",
                     linewidth = 1) +
         labs(title = "Lake Sunapee Ice-Off DOY with Regression Line", x = "Year", 
              y = "Ice-Off Day of Year (DOY)") +
         theme_bw()
```
...
Using the regression model, the predicted ice‑off day in 2040 is 102.7. Day 102.7 would be around mid April. A 90% confidence interval was made by adding the 5th and 95th percentile residuals to the prediction, giving an interval from 83.2 to 117. The interval width of 33.6 days shows the substantial year‑to‑year variability not captured by year alone.
...
prediction_2040 <- 318.94856 - 0.10602 * 2040

lower_CI <- prediction_2040 + lower_resid
upper_CI <- prediction_2040 + upper_resid
...
Ice‑off trends were compared by fitting separate regressions before and after 1970. The slope became much more negative after 1970 (−0.302 vs. −0.110 days per year), showing that ice‑off has been advancing significantly faster in recent decades, consistent with the accelerate warming of the climate.

```{r}

early <- sunapee %>% filter(year < 1970)
late  <- sunapee %>% filter(year >= 1970)

fit_early <- lm(ice_off_doy ~ year, data = early)
fit_late  <- lm(ice_off_doy ~ year, data = late)

fit_early$coefficients[2]
fit_late$coefficients[2]

```

The two regression lines shows that the post-1970 slope is substantially more negative than the pre-1970 slope. This means that for each additional year, the ice-off date is decreasing at a faster rate in the modern period. The steeper downward trend after 1970 suggests that warming effects have intensified, leading to a more rapid shift toward earlier ice melt.
...
ggplot(sunapee, aes(x = year, y = ice_off_doy)) +
  ...
  geom_line() +
  geom_abline(
    aes(intercept = fit_early$coefficients[1],
        slope = fit_early$coefficients[2],
        color = "Before 1970"),
    linewidth = 1
  ) +
  geom_abline(
    aes(intercept = fit_late$coefficients[1],
        slope = fit_late$coefficients[2],
        color = "1970 and After"),
    linewidth = 1
  ) +
  scale_color_manual(
    name = "Time Period",
    values = c("Before 1970" = "blue",
               "1970 and After" = "red")
  ) +
  labs(
    title = "Comparison of Ice-Off Trends Before and After 1970",
    x = "Year",
    y = "Ice-Off Day of Year (DOY)"
  ) +
